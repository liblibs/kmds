...
WHERE
  (
    (
      (InvoLine.Code)= Forms!PurchaseOrdersOnFile!PurchaseOrdersOnFileSubItemMovement.Form!Code
    )
    And (
      (TranTypes.Description)= Forms!PurchaseOrdersOnFile!PurchaseOrdersOnFileSubItemMovement.Form!TransType
    )
    And (
      ...
      And CDate(
        Forms!PurchaseOrdersOnFile!PurchaseOrdersOnFileSubItemMovement.Form!txtDateEnd
      )
    )
  )
ORDER BY
