...
WHERE
  (
    (
      (InvoLine.InvoHeadID)= Forms!ItemTransactionsByMonth!InvoHeadID
    )
    And (
      (InvoLine.CashSysID)= Forms!ItemTransactionsByMonth!CashSysID
    )
    And (
      (InvoLine.SetupDate)= Forms!ItemTransactionsByMonth!SetupDate
    )
  );
