                            ...
                            Begin
                                ...
                                Begin ListBox
                                    ...
                                    RowSource ="SELECT OrdHead.ID, IIf(([qryClubOrderLineTotals].[ExtPrice]-[qryClubOrderLineTot"
                                        ...
                                        "NAME]) AS ClubName, Format([InvDate],\"mm/dd\") AS [Date], OrdHead.InvoiceNo AS "
                                        "[Invoice #], qryClubOrderLineTotals.ExtPrice-qryClubOrderLineTotals.ExtDiscount "
                                        "AS Amount, OrdHead.PaidFlag, Club.ID FROM (Club INNER JOIN OrdHead ON Club.ID = "
                                        "OrdHead.ClubID) INNER JOIN qryClubOrderLineTotals ON OrdHead.ID = qryClubOrderLi"
                                        "neTotals.OrdheadID WHERE (((OrdHead.ID)<>[Forms]![ClubOrderCompare]![txtPurchase"
                                        "OrderID_1])) ORDER BY IIf(([qryClubOrderLineTotals].[ExtPrice]-[qryClubOrderLine"
                                        "Totals].[ExtDiscount])=[OrdDollars],[Club].[Name],\"**** REPRINT **** \" & [cLUB"
                                        "].[NAME]), Format([InvDate],\"mm/dd/yy\");"
                                    ColumnWidths ="0;2160;720;720;1080;1080"
