            ...
            Begin
                ...
                Begin ListBox
                    ...
                    RowSource ="SELECT AgentTransferHead.ID, [FirstName] & \" \" & [LastName] AS Name, Format([O"
                        "rdDate],\"mm/dd\") AS [Date], AgentTransferHead.ID AS InvNumber, Sum(AgentTransf"
                        "erLines.QuantityShipped) AS SumOfQuantityShipped, Sum([QuantityShipped]*[Price])"
                        " AS Dollars FROM (Agents INNER JOIN AgentTransferHead ON Agents.AgentNumber=Agen"
                        "tTransferHead.AgentID) LEFT JOIN AgentTransferLines ON AgentTransferHead.ID=Agen"
                        "tTransferLines.AgentTransferHeadID WHERE (((AgentTransferHead.Status)=\"O\")) GR"
                        "OUP BY AgentTransferHead.ID, [FirstName] & \" \" & [LastName], Format([OrdDate],"
                        "\"mm/dd\"), AgentTransferHead.ID ORDER BY [FirstName] & \" \" & [LastName], Form"
                        "at([OrdDate],\"mm/dd\"); "
